...
<div class="container">
    ...
    <div class="machine">
        ...
        <div class="payouts">
            Three 7s: 10× · Three matching symbols: 5× · Two matching symbols: 1.5×
        </div>
        ...
        .roulette-wheel {
            ...
            background:
                repeating-conic-gradient(
                    from -4.86deg,
                    #15803d 0deg 9.7297deg,
                    #111827 9.7297deg 19.4594deg,
                    #dc2626 19.4594deg 29.1891deg
                );
            transform: rotate(0deg);
            transition:
                transform 5.5s cubic-bezier(.08,.75,.15,1);
        }
...
<div class="container">
    ...
    <div class="wheel-box">
        ...
        <div
            class="roulette-area"
            id="rouletteArea"
            data-result="{{ number if number is not none else '' }}"
        >
        ...
        <div
            id="resultNumber"
            class="number {% if result_color_class %}{{ result_color_class }}{% endif %}"
        >
        ...
        <form
            method="POST"
            action="/gamble/roulette/spin"
            id="rouletteForm"
        >

            <p>
                Bet how many credits?
            </p>
        ...
        <div class="payouts">
            Red/Black/Odd/Even: 2× total payout ·
            Specific number: 36× total payout
        </div>
...
const segment = 360 / wheelNumbers.length;
...
function toggleNumberBet() {

    const betType =
        document.getElementById("betType").value;

    const numberBet =
        document.getElementById("numberBet");

    if (betType === "number") {
        numberBet.style.display = "inline-block";
    } else {
...
const spinningText =
    document.getElementById("spinningText");

const form =
    document.getElementById("rouletteForm");
...
if (result !== "") {
    ...
    const winningNumber = Number(result);

    const winningIndex =
        wheelNumbers.indexOf(winningNumber);

    if (winningIndex !== -1) {

        resultNumber.style.visibility = "hidden";
        ...
        setTimeout(function() {
            ...
            resultNumber.style.visibility =
                "visible";

            resultLabel.style.visibility =
                "visible";
...
<div class="container">
    ...
    <div class="table">
        ...
        <div class="note">
            Your casino credits are your normal StudySpace credits.
        </div>
...
def roulette_spin():
    ...
    color_name, color_class = roulette_color(
        result
    )
    ...
    return render_template_string(
        ...
        result_color_class=color_class,
        message=message,
        error=None
    )
...
def poker_hand_result(cards):
    ...
    if (
        is_straight
        and is_flush
        and max(unique_values) == 14
        and min(unique_values) == 10
    ):
...
def poker_draw():
    ...
    hand_name, multiplier = poker_hand_result(
        game["cards"]
    )
